...
cargarListaEmpresa(){
  if(this.regionalSeleccionada){
    this.usuarioService.listaEmpresa(this.regionalSeleccionada)
    .subscribe((respuesta: any) => {
      console.log("respuesta",respuesta.data);
      if(respuesta.status){
        console.log(respuesta.data);
        this.listaEmpresa=respuesta.data;
      }
    });
  }
}
mostrarDialogoRestriccionGuardar(){
  ...
  abrirEditarRestriccion(r:any){
    ...
    this.regionalSeleccionada=r.codDepartamento;
    this.editarRestriccionDialogo=true;
    ...
    .subscribe((respuesta: any) => {
      ...
      this.nivelForm.controls['codDepartamento'].setValue(this.regionalSeleccionada);
      this.nivelForm.controls['idEmpresa'].setValue(r.idEmpresa);
    });
  }
  cerrarNuevoUsuario(){
    this.editarRestriccionDialogo=false;
    this.sistemaSeleccionado=0;
    this.regionalSeleccionada=0;
    this.nivelForm.reset();
    this.perfilForm.reset();
  }
}
